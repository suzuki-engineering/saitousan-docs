...
`muse-runner` は、ユーザーの主タスク完了後に MUSE review を別コンテキストへ渡すための Codex custom agent です。
...
```text
再利用性の判定
.muse/candidates/ の作成または refinement
eval.yaml / tests の実行
usage.jsonl / .memory.md の更新
昇格条件の確認
```

本流の会話ではユーザーの依頼を優先し、長い Skill 探索や評価ログの整理は `muse-runner` に委譲します。
