...
## 1. Claude Code に読み込ませる

このリポジトリ直下に `.claude/CLAUDE.md` があるため、Claude Code でこのフォルダを開くと MUSE 運用ポリシーをプロジェクト指示として使えます。

```text
C:\Users\iocomk\Desktop\muse
```

Claude Code に依頼するときは、次のように頼むと MUSE 方針に沿いやすくなります。
...
本番 Skill として直接 `.claude/skills/` に作らず、まず `.muse/candidates/` に作ります。
...
```powershell
Copy-Item -Recurse .claude\skills\_template .muse\candidates\my-skill
```
...
現状では `eval.yaml` を実行する評価スクリプトはまだ未実装です。手動で `command` を実行して、結果を `usage.jsonl` に記録します。
...
候補 Skill を `.claude/skills/` に移す条件:
...
```powershell
Move-Item .muse\candidates\my-skill .claude\skills\my-skill
```

## 9. 外部 Skill を取り込む

外部 Skill は直接 `.claude/skills/` に入れません。
...
```text
.claude/skills/ から使える Skill を探して、このタスクに使って。結果を usage.jsonl に記録して。
```
...
現状では、MUSE の自動 router / evaluator / refiner はまだありません。

そのため、Claude Code にこのリポジトリを開かせて `.claude/CLAUDE.md` の方針で作業させる運用になります。

完全自動化するには、`muse/` 配下に実行スクリプトを実装します。
