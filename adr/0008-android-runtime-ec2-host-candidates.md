...
Runtime Hostの第一検証は、`Ubuntu Server 24.04 LTS x86_64 + c7i.4xlarge` とする。メモリ不足やEmulator/FFmpeg/Appiumの同居で余裕がない場合のみ `c7i.8xlarge` に上げる。

ただし、`c7i` はCPU-only smoke testであり、KVMまたはAndroid Emulator加速が成立しない場合は長追いしない。失敗時は原因を記録し、GPUレンダリング・映像処理・GUI切り分けの候補へ進む。

候補の優先順は次の通り。

| Priority | Candidate | OS | Purpose | Initial EBS | GPU driver | Acceptance check | Stop condition |
| --- | --- | --- | --- | ---: | --- | --- | --- |
| 1 | `c7i.4xlarge` | Ubuntu Server 24.04 LTS x86_64 | CPU-only smoke。SDK/Appium/AVD構築とKVM可否の最短確認 | 100-150 GiB gp3 | 不要 | `/dev/kvm`、`emulator -accel-check`、headless AVD boot、`adb devices`、Appium session、screenshot | KVM不可、AVD起動不可、メモリ不足が明確 |
...
| 2 | `g4dn.xlarge` / `g4dn.2xlarge` | Ubuntu Server 24.04 LTS x86_64 | NVIDIA T4でGPUレンダリング、FFmpeg、スクリーンショット、仮想カメラ周辺を確認 | 150-200 GiB gp3 + instance store利用可否を確認 | 必要 | NVIDIA driver、Emulator rendering、Appium session、screenshot、FFmpeg probe | driver/Emulator renderingが詰まる、KVM不可 |
| 2b | `g5.xlarge` / `g6.xlarge` | Ubuntu Server 24.04 LTS x86_64 | `g4dn` がquota/availability/性能で不十分な場合のGPU代替 | 150-200 GiB gp3 | 必要 | `g4dn` と同じ。A10G/L4世代差で改善するか | コストまたはquotaがPoCに不向き |
| 3 | `g4dn.metal` などのbare metal | Ubuntu Server 24.04 LTS x86_64 | 通常EC2 VMで仮想化要件が詰まる場合の切り分け | 200 GiB gp3以上 | 必要 | bare metalでKVM/Emulatorブロックが解消するか | 2-4hで解消しない、費用が大きい |
| 4 | `g4dn.xlarge` / `g5.xlarge` / `g6.xlarge` + Windows Server | Windows Server | Linux headlessでGUIデバッグが不足する場合の逃げ道 | 200 GiB gp3以上 | 必要 | Android Studio GUIでAVD、Appium、screenshot、カメラ入力の切り分け | GUIでしか進まない理由が説明できない |
...
- EC2検証を「CPU-only smoke → GPU Linux → bare metal → Windows GUI」の順に絞れる。
...
- Android EmulatorはクラウドVM上のネスト仮想化やKVM公開状況に影響を受けるため、インスタンススペックだけでは成否を判断できない。
